...
def render_global_performance():
    ...
    tb_backtest = st.session_state.get('trade_book_backtest', pd.DataFrame())
    ...
    tracking_error = Display.calculate_tracking_error(tb, tb_backtest)
    
    # Calculate strategy vs asset performance and volatility
    strategy_return, asset_return = Display.calculate_strategy_vs_asset_performance(tb, md)
    strategy_volatility, asset_volatility = Display.calculate_strategy_vs_asset_volatility(tb, md)
    ...
    avg_duration = tb["duration_min"].mean()
    total_trades = len(tb)
    
    # Calculate total fees paid
    # Try to find fee columns first, otherwise estimate from trade volume
    total_fees = 0.0
    if "Fee" in tb.columns:
        total_fees = tb["Fee"].sum()
    elif "Commission" in tb.columns:
        total_fees = tb["Commission"].sum()
    elif "Fees" in tb.columns:
        total_fees = tb["Fees"].sum()
    else:
        # Estimate fees based on typical trading fees (0.1% per trade)
        # This is a rough estimate - you may want to adjust the fee rate
        total_volume = tb["size_usdt"].sum() if "size_usdt" in tb.columns else 0
    ...
    filtered_tb = tb
    filtered_tb_backtest = tb_backtest
    filtered_md = md
    
    
    fig = Display.equity_curve_with_backtest(filtered_tb, filtered_tb_backtest, filtered_md, None)
    fig.update_layout(
    ...
    if "Confidence" in tb.columns or "Score" in tb.columns or "Signal" in tb.columns:
        st.markdown("""
        <h3 style="
            color: #ffffff;
            margin: 0 0 0.5rem 0;
        ...
        fig = Display.expectation_vs_reality(tb)
        if fig is not None:
            fig.update_layout(
                height=350,
