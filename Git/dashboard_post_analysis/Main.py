...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Load and cache data files from Excel sources.
    
    Returns:
        Tuple of (trade_book, trade_book_backtest, market_data) DataFrames
    """
    # Validate file existence and size
    for file in REQUIRED_FILES:
        if not os.path.exists(file):
            st.error(f"File {file} does not exist in the current directory.")
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
        
        if os.path.getsize(file) == 0:
            st.warning(f"File {file} is empty (0 bytes). This may cause issues.")
    
    # Try loading with different Excel engines
    for engine in EXCEL_ENGINES:
        try:
            trade_book = pd.read_excel('Trade_report.xlsx', engine=engine)
            trade_book_backtest = pd.read_excel('Trade_report_backtest.xlsx', engine=engine)
            market_data = _load_market_data(engine)
            
            st.success(f"Successfully loaded data with engine: {engine}")
            return trade_book, trade_book_backtest, market_data
            
        except Exception as e:
            st.write(f"Failed with engine {engine}: {str(e)}")
            continue
    
    # Fallback: try without specifying engine
    try:
        trade_book = pd.read_excel('Trade_report.xlsx')
        trade_book_backtest = pd.read_excel('Trade_report_backtest.xlsx')
        market_data = _load_market_data()
        
        st.success("Successfully loaded data without specifying engine")
        return trade_book, trade_book_backtest, market_data
        
    except Exception as e:
        st.error(f"Failed to load Excel files: {str(e)}")
        st.error("Please ensure the Excel files are in the correct directory and format.")
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
