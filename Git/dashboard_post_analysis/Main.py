...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Load and cache Excel files (Trade_report, Trade_report_backtest, Market_data).
    Works both locally and on Streamlit Cloud.

    Returns:
        Tuple of (trade_book, trade_book_backtest, market_data)
    """
    # Define absolute paths
    trade_report_path = BASE_DIR / "Trade_report.xlsx"
    trade_report_backtest_path = BASE_DIR / "Trade_report_backtest.xlsx"
    market_data_path = BASE_DIR / "Market_data.xlsx"

    # Verify file existence and size
    for path in [trade_report_path, trade_report_backtest_path, market_data_path]:
        if not path.exists():
            st.error(f"File {path.name} does not exist in {BASE_DIR}")
            st.stop()
        if path.stat().st_size == 0:
            st.warning(f"File {path.name} is empty. This may cause issues.")

    # Helper to read Excel with normalization
    def read_excel(path: Path) -> pd.DataFrame:
        return pd.read_excel(unicodedata.normalize('NFC', str(path)), engine="openpyxl")

    try:
        trade_book = read_excel(trade_report_path)
        trade_book_backtest = read_excel(trade_report_backtest_path)
        market_data = read_excel(market_data_path)
